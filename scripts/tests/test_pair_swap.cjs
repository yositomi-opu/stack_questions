...
assert.deepEqual(rows.map(r=>r.truth),['C','C','W','C','W']);
[1,2,0,3,4].forEach((oldIndex,i)=>assert.deepEqual({...rows[i],truth:original[oldIndex].truth}, original[oldIndex]));
assert.equal(Object.keys(state.casEvaluation.expressions).length,0);
context.swapPairedOptions('01');
assert.deepEqual(JSON.parse(JSON.stringify(rows)),original);
context.swapPairedOptions();
assert.equal(rows[3].choice_ja,'E');
assert.equal(rows[4].choice_ja,'D');
assert.equal(rows[3].feedback_ja,'shared');
assert.deepEqual(rows.map(r=>r.truth),['C','C','W','C','W']);
context.swapPairedOptions();
assert.deepEqual(JSON.parse(JSON.stringify(rows)),original);
