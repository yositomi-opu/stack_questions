...
assert.equal(context.syncTranslationTarget(),'all','base language is not a target; default to all');
el.translationTarget.value='';
context.prepareTranslationRequest();
const allRequest=el.translationJson.value;
const allInput=JSON.parse(allRequest.slice(allRequest.indexOf('\n{\n')+1));
assert.deepEqual(allInput.target_languages,['en','pt']);
assert.match(allRequest,/Filename: mcq-translations-all.json/);
assert.match(allRequest,/1つのJSONファイル/);
assert.match(allRequest,/const protectedParts = function translationProtectedParts/);
assert.match(allRequest,/数式の区切りと中身は一体/);
assert.match(allRequest,/直すのは訳文だけ/);
assert.match(allRequest,/原文を手入力で再構成/);
assert.match(allRequest,/そのファイルを読み直し/);
assert.ok(!allRequest.includes('指定された1言語だけ'));
assert.equal(context.syncTranslationTarget(),'all');
