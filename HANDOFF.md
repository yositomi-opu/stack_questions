...
- 初期文言: タイトルのplaceholderと未入力時の保存ダイアログ例を `MCQ_sampleXYZ` へ変更。初期問題・同梱CSV・天体XMLサンプルの問題文を「次の恒星・惑星・衛星に関する主張について、__SELPROMPT__」へ変更し、英語も対応。`__SELPROMPT__` 自体が「正しいものを」を含むため前置きには重複させない。固定真偽モードのCSV見本は数学の内容なので、方程式の解に関する文言を使う。
