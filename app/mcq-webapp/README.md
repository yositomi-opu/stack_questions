...
XMLには、入力形式や各種設定を復元する再編集用メタデータを保存します。XMLを直接編集して問題変数の`%__mcq_qtextL`がメタデータと異なる場合、読込時にXML側の問題文を優先し、通知します。静的な`castext("…")`はCASText入力として復元します。再保存するとメタデータも更新されます。選択肢・フィードバックもXML内の定義から復元します。CASリスト式としてまとめて復元される場合があります。アプリ生成XMLの問題変数本体や、取得できたincludeの内容もファイルを優先します。パラメータ等の画面設定は引き続きメタデータから復元します。CSV・XML・見本の読込完了後は問題変数を自動評価します。評価に失敗しても読み込んだ内容は保持され、評価ログで原因を確認できます。
...
フィードバックはCASTextとCAS式を選べます。CAS式のフィードバックもsconcat・tex2などの既知の形式は出力時に変換し、独自の式や既存のcastext()は保持します。自動変換には限界があるため、可能であれば手動でCASTextへ変更してください。選択肢の文字列・CAS式・CASリスト式は従来どおりです。問題変数の最後に選択言語の`%__mcq_langcode`を生成し、STACKの言語判定用に問題文にも空の言語ブロックを付けます。Moodleで使う前に、更新した`*_cas.mac`を共通ライブラリの公開先へ配置してください。
