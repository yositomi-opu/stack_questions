...
一対モードのXMLでは、パターンごとに `optC1`／`msgC1`、`optW1`／`msgW1` などを定義します。各定義の `%__mcq_lang([["ja", ...], ["en", ...]], %_STACK_LANG)` で言語を選び、候補一覧 `%__mcq_Csource`／`%__mcq_Wsource` とフィードバック一覧には変数名だけを並べます。単独候補は `[optC1]`、候補リストは `optC2` のように渡し、CASText内部のリスト構造は展開しません。抽出結果は `%__mcq_Cpatterns`／`%__mcq_Wpatterns` 等を通してテンプレートへ渡します。`MCQ_CHOICES_BEGIN`〜`MCQ_CHOICES_END` はアプリが更新する範囲なので、コメントを保持してください。旧形式のXMLも読み込めます。一対でない場合は各パターンの候補リストを直接テンプレートへ渡し、同一パターンから複数候補を出題できます。
...
`optionsL`、`ListAL1`、`makelist(...)` などは画面では「CAS式」と表示します。既存ファイルのリスト指定、または問題変数内のリスト定義から複数候補としての扱いを保持します。互換性のため、CSV内部ではリストを返す式を従来の `cas_list` として記録する場合があります。変数名の参照先は自動で書き換えません。`makelist` の本文が定数文字列だけの `sconcat` なら変換しますが、ループ変数を使う式などは評価タイミングを変えないよう保持します。リストの各要素が適切な文字列／CASTextかは編集者が確認してください。参照先が外部includeにある場合は既存のリスト指定を保持してください。

一対モードの `optC番号`／`optW番号`／`msgC番号`／`msgW番号` は自動生成用の変数名です。問題変数や補助パラメータで同名を定義しないでください。XMLを直接編集した場合は、各変数の言語別定義を優先して読み込みます。旧形式の `CsourceL`／`WsourceL` を含むXMLも引き続き読み込めます。
