...
画面上部のCSV／XMLメニューから「見本」「読み込み」「表示」「保存」を選べます。CSVの「見本」は `samples.ja` と `sample.csv`、XMLの「見本」は `001` と `samples` にあるMCQの実ファイルを選んで読み込みます。CSVの「保存」は再編集可能なCSVを保存します。保存するCSVと同梱サンプルにはUTF-8 BOMを付けているため、Excelから直接開いても日本語が文字化けしません。
