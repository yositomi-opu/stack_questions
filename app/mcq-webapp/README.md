...
フィードバックはCASTextとCAS式を選べます。選択肢の文字列・CAS式・CASリスト式は従来どおりです。問題変数の最後に選択言語の`%__mcq_langcode`を生成し、STACKの言語判定用に問題文にも空の言語ブロックを付けます。Moodleで使う前に、更新した`*_cas.mac`をinclude URLベースの公開先へ配置してください。
