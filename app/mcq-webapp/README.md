...
問題文の「CAS変換」は、現在表示している言語の文字列形式の問題文を対象に、`{@a@}` を `stack_disp(a, "i")` に置き換え、前後の文章を `sconcat(...)` で連結するCAS式に変換します。変換後は入力形式もCAS式になります。ボタンにカーソルを置くかフォーカスすると説明を表示します。
