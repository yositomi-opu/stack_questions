...
選択肢数の最大値は、文字列1件を1候補、評価済みCASリストをその`length`件として、利用可能な候補数に合わせて自動調整されます。同じパターンのCASリストから複数の選択肢を生成できます。問題文は右欄の選択肢設定の下に表示されます。見出し横の「？」にカーソルを置くかフォーカスすると、文字列／CAS式形式の説明が表示されます。「挿入」からSELTYPEまたはSELPROMPTを選ぶと、現在の問題文のカーソル位置に挿入します（選択範囲があれば置換）。各記号と挿入操作の説明も「？」にまとめています。CAS式では文字列の中に挿入してください。CSV／XMLの「表示」は読み取り専用の別画面で開き、右上のコピーボタンで全文をコピーできます。左右の初期幅は4:6です。境界線のドラッグや「表示設定」で左欄をさらに広げられます。「表示設定」では設定欄・選択肢欄の幅を変更でき、同じブラウザ・同じサイトでは次回も幅を復元します。表示言語は右上の地球アイコン横のプルダウンで日本語／Englishを選択します。

CSV／XMLを読み込むと、前の評価結果・エラーログ・翻訳依頼を消去します。CSVにパラメータの指定がなければ空欄になります。XMLのinclude読込後の自動評価は、新しく読み込んだ問題を対象に実行します。「表示設定」の左にある「全入力クリア」は、確認後にタイトル・パラメータ・問題変数・全言語の問題文・選択肢・フィードバック・評価結果を空にし、問題設定を初期値に戻します。表示幅・表示言語・接続先設定は保持します。正解・不正解一対の初期値はオフで、ファイルに明示された設定があればそれを復元します。
...
問題変数の評価ボタンは見出し右側にあります。間の「？」で選択肢設定・パラメータとの関係を確認できます。別ファイル保存とURLベースは左欄の問題変数の下にまとめています。チェック横の「？」で公開先の指定方法を確認してください。XMLとincludeは個別に保存し、includeはXMLが参照する公開URLに配置します。
...
CASText版の問題文にはSTACKと同じ書式で文章、`{@a@}`、ブロック構文を入力します。外側の引用符や`castext()`はアプリが付けます。従来の文字列はそのまま、CAS式の問題文は、外側の`sconcat()`を分解して文章と`{@...@}`へ自動移行します。通常の`tex2()`・`stack_disp(...,"i")`や、直接書いたリストの`tex2L()`を変換し、独自処理や動的リストはその部分だけ式を埋め込んで保持します。これはCASTextへの一方向変換です。`__SELTYPE__`／`__SELPROMPT__`も選択指示に対応します。移行後の問題文を編集せずにチェックをオフにした場合は、元のCAS式へ戻します。復元用情報もCSVとXMLに保存します。

通常版でも問題文の種別に「CASText」を選べます。選ぶと「castext 版を使用する」が自動でオンになり、フィードバックも必要に応じてCASText形式へ変換するよう案内します。手動で書いた内容は再変換せずに使用します。チェックを手動で外した場合、CASTextの問題文のXML生成・プレビューには再度チェックが必要です。CSVのqtextL行の型`castext`でこの状態を保存します。

フィードバックはCASTextとCAS式を選べます。CAS式のフィードバックもsconcat・tex2などの既知の形式は出力時に変換し、独自の式や既存のcastext()は保持します。自動変換には限界があるため、可能であれば手動でCASTextへ変更してください。CASText版では選択肢の文字列も `castext()` で包んでXMLへ保存します。入力欄には文章や `{@a@}` を直接書き、外側の `castext()` は付けません。CAS式とCASリスト式は式のまま保存します。CSVでは入力した本文と型を保持します。問題変数の最後に選択言語の`%__mcq_langcode`を生成し、STACKの言語判定用に問題文にも空の言語ブロックを付けます。Moodleで使う前に、更新した`*_cas.mac`を共通ライブラリの公開先へ配置してください。

選択肢のCASリスト式は、リストの各要素を1つの選択肢として扱います。CASTextを返す関数で作ったリストも使用できます。複数のリストは外側だけ連結し、CASTextやリスト値の内部を分解しません。旧版で生成したXMLに`flatten([ListAL1])`等がある場合は、更新後のアプリで読み込み直してXMLを保存してください。

翻訳結果の貼付では、JSONのみ、コードブロック付き、説明文付きの回答を受け付けます。依頼文が一緒に残っていても、有効な`translations`を含むJSONが1つなら抽出します。依頼文だけの場合は回答への貼り替えを案内します。JSONが複数ある場合や途中で切れた場合、引用符が不正な場合は反映せず、修正方法を表示します。数式やバックスラッシュの内容は自動で書き換えません。

基本言語はXMLの展開対象に必ず含まれ、チェックを解除できません。基本言語を変更すると、元の言語のチェックは操作できるようになります。展開先にはスペイン語（es）も選べます。

「ファイル名：」欄で名前を指定し、右端の「保存」でincludeファイルを取得します。入力欄は編集エリアの幅に合わせて伸縮します。「URLベース」には公開ディレクトリを直接指定してください。右のグレーのファイル名と連結したURLを使用し、001/は自動追加しません。既定値には001/を含め、旧設定も従来の公開先になるよう移行します。既定はタイトルから末尾の`-rb`／`-cb`を除いた名前に`.txt`を付けたものです。手動指定した名前はタイトルを変更しても保持され、CSVとXMLの再編集用データに保存されます。XML内のinclude参照先にも同じ名前を反映します。

Radioでは「正解が2つ以上の場合も1つ選べばよい」を使用するチェックで補足説明を指定できます。オンではXMLに`%__mcq_rb_cb:"rb2";`、オフでは`"rb"`を出力します。正解数の抽選結果で説明は変わりません。Checkboxではこの設定を無効にし、常に`"cb"`を出力します。設定はCSVとXMLの再編集用データに保存します。通常版のpostもこの指定に従うため、従来の`rb`指定だけでは複数正解時の補足説明は自動付加されません。
