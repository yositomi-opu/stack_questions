...
正解・不正解一対モードでは、各パターンの「正↔不」でそのパターンを、表の上の「全体：正↔不」で全パターンを反転できます。全言語の選択肢とフィードバック、入力形式をまとめて保持します。共通フィードバックはそのままです。もう一度押すと元に戻ります。CAS評価結果はクリアされるので必要に応じて再評価してください。問題文の選択条件は自動変更しません。
