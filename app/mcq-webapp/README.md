...
問題変数の評価ボタンは見出し右側にあります。間の「？」で選択肢設定・パラメータとの関係を確認できます。別ファイル保存とinclude URLベースは左欄の問題変数の下にまとめています。チェック横の「？」で公開先の指定方法を確認してください。XMLとincludeは個別に保存し、includeはXMLが参照する公開URLに配置します。
...
フィードバックはCASTextとCAS式を選べます。CAS式のフィードバックもsconcat・tex2などの既知の形式は出力時に変換し、独自の式や既存のcastext()は保持します。自動変換には限界があるため、可能であれば手動でCASTextへ変更してください。選択肢の文字列・CAS式・CASリスト式は従来どおりです。問題変数の最後に選択言語の`%__mcq_langcode`を生成し、STACKの言語判定用に問題文にも空の言語ブロックを付けます。Moodleで使う前に、更新した`*_cas.mac`をinclude URLベースの公開先へ配置してください。
...
「include保存」の横の入力欄で、保存するファイル名をあらかじめ指定できます。既定はタイトルから末尾の`-rb`／`-cb`を除いた名前に`.txt`を付けたものです。手動指定した名前はタイトルを変更しても保持され、CSVとXMLの再編集用データに保存されます。XML内のinclude参照先にも同じ名前を反映します。
