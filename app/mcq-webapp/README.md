...
パターン番号の上限は、使用するXMLテンプレートの `%__mcq_max_cp`／`%__mcq_max_wp` から取得します。一対モードの上限は両者の小さい方です。現在のCASText版は正解・不正解とも15です。通常版は通常版テンプレートの上限に従います。CASリストの評価後は、入力形式の左に `length:件数` を表示します。
