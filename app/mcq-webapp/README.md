...
CSV／XMLを読み込むと、前の評価結果・エラーログ・翻訳依頼を消去します。CSVにパラメータの指定がなければ空欄になります。XMLのinclude読込後の自動評価は、新しく読み込んだ問題を対象に実行します。「表示設定」の左にある「クリア」は、確認後にタイトル・パラメータ・問題変数・全言語の問題文・選択肢・フィードバック・評価結果を空にし、問題設定を初期値に戻します。表示幅・表示言語・接続先設定は保持します。正解・不正解一対の初期値はオフで、ファイルに明示された設定があればそれを復元します。
