...
問題文・言語依存の選択肢・フィードバックを選択したAIへ送信します。問題変数の定義やAPIキーを翻訳本文へ含めることはありません。内部では1言語・最大4行ずつに分割し、1言語の全結果の検証が済んでから反映します。数式・STACK埋込・プレースホルダー・HTMLタグは送信前に保護マーカーへ置き換え、翻訳後に元の文字列へ戻します。候補ID、翻訳漏れ、保護マーカーの欠落・重複と復元後の構文を検査し、不一致の場合は言語・バッチ・項目を表示して止めます。翻訳の意味や正誤判定まで保証する検査ではないので、内容を確認してください。
...
1. 「ファイルで翻訳」を開き、「今回の翻訳先」を1言語選びます。
2. 「依頼をコピー」でAIのチャットへ渡すか、「依頼を保存」でテキストファイルを保存してAIへ添付します。
3. AIに、画面への長い出力ではなく、ダウンロード可能なUTF-8の `mcq-translations-en.json` 等を作成してもらいます。ファイルを生成・ダウンロードできるAI環境が必要です。
4. ファイルをダウンロードし、「翻訳JSONを読込」で選択します。反映後は編集画面の訳文を確認してください。
5. 次の言語へ切り替えて繰り返します。他の言語の翻訳は保持されます。
...
ファイルへの出力もAIの生成制限をなくすものではありません。アプリは4MBまでのJSONについて、構文・各言語の問題文・必要な候補ID・翻訳漏れ・保護する数式やタグを検査し、どれかが不正ならファイル全体を反映しません。原文を変更した後は、必要な言語の翻訳をすべて更新するまでXML生成を保留します。翻訳の意味の正確さは編集者が確認してください。
