...
XMLには言語別の候補一覧 `%__mcq_CsourceL`／`%__mcq_WsourceL` とフィードバック一覧を一度だけ記述し、抽出結果を `%__mcq_Cpatterns`／`%__mcq_Wpatterns` 等を通してテンプレートへ渡します。`MCQ_CHOICES_BEGIN`〜`MCQ_CHOICES_END` はアプリが更新する範囲なので、コメントを保持してください。旧形式のXMLも読み込めます。一対でない場合は各パターンの候補リストを直接テンプレートへ渡し、同一パターンから複数候補を出題できます。
