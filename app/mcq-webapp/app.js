...
function problemVariableNames() {
  ...
  return [...new Set(
    splitMaximaStatements(stripMaximaComments(evaluationVariableCode()))
...
function generateVariableBlock(includePreamble = true) {
  ...
  return generateFixedVariableBlock(patterns, numOptions, counts, includePreamble);
}
...
function baseVariableLines(numOptions, counts, includePreamble = true) {
  return [
    ...(includePreamble ? parameterPreamble() : []),
    "",
...
function importLegacyQuestionVariables(variables, documentNode, filename, xmlVariables = variables) {
  ...
  const numOptions = preamble.settings["%_MCQ_NUM_OPTS"] || variables.match(/%_MCQ_NUM_OPTS\s*:\s*(\d+)/)?.[1];
  const correctSetting = preamble.settings["%_MCQ_NUM_COPTS"]
    || variables.match(/%_MCQ_NUM_COPTS\s*:\s*(rand\s*\(\s*\[[^\]]+\]\s*\)|\d+)/)?.[1] || "1";
  el.numOptions.value = numOptions || String(Math.min(2, patternCount));
  const correctCounts = correctSetting.match(/^rand\s*\(\s*\[([^\]]+)\]\s*\)$/)?.[1];
  el.numCorrect.value = /^\d+$/.test(correctSetting) ? correctSetting : "1";
  el.randomCorrect.checked = Boolean(correctCounts);
  el.correctCounts.value = correctCounts?.trim() || "1, 2";
...
function legacyIncludePreamble(xmlVariables) {
  const main = extractMainVariableSection(xmlVariables);
  const match = /\bstack_include\s*\(/.exec(maskMaximaCommentsAndStrings(main));
  if (!match) return { parameters: "", settings: {} };
  const prefix = main.slice(0, match.index);
  const settings = {};
  const parameters = [];
  let start = 0;
  while (start < prefix.length) {
    const end = scanMaximaStatementEnd(prefix, start);
    const statement = prefix.slice(start, end);
    const parsed = parseMaximaAssignment(stripMaximaComments(statement));
    if (parsed && ["%_MCQ_NUM_OPTS", "%_MCQ_NUM_COPTS"].includes(parsed.name)) {
      const supported = parsed.name === "%_MCQ_NUM_OPTS" ? /^\d+$/ : /^(?:\d+|rand\s*\(\s*\[[\d,\s]+\]\s*\))$/;
      if (!supported.test(parsed.expression)) throw new Error(`選択肢設定を解析できません: ${parsed.raw}`);
      settings[parsed.name] = parsed.expression;
      const offset = maskMaximaCommentsAndStrings(statement).indexOf(parsed.name);
      parameters.push(statement.slice(0, offset));
