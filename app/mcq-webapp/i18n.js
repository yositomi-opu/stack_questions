(() => {
  ...
  const translations = new Map(Object.entries({
    ...
    "CASText版のフィードバックでは、CASTextに文章や{@a@}、ブロック構文を入力できます。外側のcastext()は不要です。CAS式のsconcatやtex2などはできる範囲で変換し、独自の式はそのまま使用します。CASText版では、選択肢の「文字列」もCASTextとして処理します。文章や{@a@}を入力し、外側のcastext()は付けません。「CAS式」は1選択肢、「CASリスト式」は複数候補を返す式です。": "For CASText feedback, enter text, {@a@}, or blocks without an outer castext() call. Legacy sconcat and tex2 feedback expressions are converted where possible; custom expressions remain unchanged. In CASText mode, string choices are also processed as CASText. Enter text or {@a@} without an outer castext() call. A CAS expression represents one choice; a CAS list expression supplies multiple candidates.",
